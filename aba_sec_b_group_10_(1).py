...
# prompt: this error is show for installing the pip encoder coorect it for streamlit file run. File "/mount/src/room-price-prediction/aba_sec_b_group_10_(1).py", line 484
#   !pip install category_encoders
#   ^
...
# prompt: make code to this example -mport subprocess
# subprocess.check_call([sys.executable, "-m", "pip", "install", "category_encoders"]) in my file
...
# The code to save the model and encoder is already present in the provided code.
# No additional code is needed to save them.  The code to load is also present.

# However, if you want to save the model to a different file, you can change the filename in the following lines:
# filename = 'my_new_model_name.sav'
# pickle.dump(lm, open(filename, 'wb'))

# filename = 'my_new_encoder_name.sav'
# pickle.dump(encoder, open(filename, 'wb'))
